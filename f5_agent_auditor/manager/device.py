...
class Device(object):
    ...
    def get_dev_config_of_vlans(self):
        ret = {}

        vlans = self.bigip.get_vlans()
        for vlan in vlans:
            ifaces = vlan.interfaces_s.get_collection()
            vlan.interfaces = {
                iface.fullPath: iface.attrs for iface in ifaces}
            vlan_attr = vlan.attrs
            vlan_attr.pop('interfaces_s')
            ret[vlan.fullPath] = vlan_attr
